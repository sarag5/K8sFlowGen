...
def generate_flowchart_data(yaml_file):
    """Generate flowchart data from a Kubernetes YAML file."""
    yaml_data = parse_yaml(yaml_file)
    dot = create_flowchart(yaml_data)
    
    # Convert the dot object to a JSON-serializable format
    nodes = [{'id': node, 'label': node} for node in dot.nodes()]
    edges = [{'from': edge[0], 'to': edge[1]} for edge in dot.edges()]
    
    return json.dumps({'nodes': nodes, 'edges': edges})

if __name__ == '__main__':
    yaml_file = sys.argv[1]
    print(generate_flowchart_data(yaml_file))